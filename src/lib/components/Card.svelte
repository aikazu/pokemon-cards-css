<script>
	...
	import {
		getPointerSpringUpdate,
		getOrientationSpringUpdate,
	} from "../helpers/cardInteraction.js";
	...
	const interactEnd = (e, delay = 500) => {
		// Default delay before snap-back animation
  ...
  `;
	 ...
	 */
	const orientate = (e) => {
		const springUpdate = getOrientationSpringUpdate(
			e.relative.gamma,
			e.relative.beta,
		);
		updateSprings(
			springUpdate.background,
			springUpdate.rotate,
			springUpdate.glare,
		);
	};
